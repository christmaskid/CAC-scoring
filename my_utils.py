...
from monai.inferers.utils import _get_scan_interval, _flatten_struct, _pack_struct, _create_buffered_slices, _compute_coords
...
def my_sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        if with_coord:
            seg_prob_out = predictor(win_data, unravel_slice, *args, **kwargs)[0]  # batched patch ### HERE
        else:
            seg_prob_out = predictor(win_data, *args, **kwargs)[0]  # batched patch ## HERE
