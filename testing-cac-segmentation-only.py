...
model.load_state_dict(torch.load("/home/cvai_607/radiology_CAC/model/segmentation_new2_cac_no_rotation.pt")) # <-貼上要測試的model路徑
model.to(device)
model.eval()

## DATASET: PASTE THE DESIRED DIRECTORY
data_dir = "/raid/data/cvai_607/CAC_100_segmentation/" # <- 貼上要測試的檔案路徑；架構是資料夾底下有 images/ 和 labels/ ，可以調整。
data_list = []
...
for file in os.listdir(data_dir+"labels/"):
    ...
    data_list.append({
        "image-name": filename,
        "image": os.path.join(data_dir, "images", filename+".nii.gz"),
        "label": os.path.join(data_dir, "labels", file)
    })
print(len(data_list), data_list[0], flush=True)
